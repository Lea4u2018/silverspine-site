...
function followUpLabel(key) {
  if (key === "manualOutside") return "Emailed (marked in hub)";
...
export function buildLaunchActivityLog(store) {
  ...
  for (const [email, note] of Object.entries(store?.trackingNotes || {})) {
    if (!note?.emailedAt) continue;
    pushEvent(entries, {
      name: note.name || email,
      email,
      personId: email,
      recordType: "tracking",
      source: "Contact hub",
      ...
      kind: "sent",
      at: note.emailedAt,
      subject: note.emailedSubject || "",
      label: "Emailed (marked in hub)",
      followUpKey: "manualOutside",
    });
  }
