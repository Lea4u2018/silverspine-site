...
export async function saveTrackingNote({ email, note }) {
  ...
  const next = await writeStore((store) => {
    ...
    const prev = store.trackingNotes[key] || {};
...
export async function markPersonEmailed({ email, name }) {
  const key = String(email || "")
    .trim()
    .toLowerCase();
  if (!key) throw new Error("Email is required.");
  ...
  const next = await writeStore((store) => {
    ...
    store.trackingNotes[key] = {
      ...prev,
      note: String(prev.note || "").trim(),
      name: String(name || prev.name || "").trim(),
      emailedAt: now,
      emailedSubject: "Marked emailed (outside Admin)",
      updatedAt: now,
    };
    return store;
  }, `launch-admin: mark emailed ${key}`);
  return next.trackingNotes[key];
}
...
export async function addRecipient(input) {
  ...
  await writeStore((store) => {
    ...
    const row = {
      ...
      name,
      email,
      role: ["arc-applicant", "arc-selected", "launch-list", "giveaway", "sneak-peek", "manual"].includes(input.role)
        ? input.role
        : "manual",
...
export async function importSubmissions(ids, role) {
  ...
  if (!["arc-applicant", "arc-selected", "launch-list", "giveaway", "sneak-peek", "manual"].includes(role)) {
...
export async function updateRecipient(id, patch) {
  ...
  await writeStore((store) => {
    ...
    if (patch.name != null) cur.name = String(patch.name).trim();
    if (patch.email != null) cur.email = String(patch.email).trim().toLowerCase();
    if (patch.role != null && ["arc-applicant", "arc-selected", "launch-list", "giveaway", "sneak-peek", "manual"].includes(patch.role)) {
      cur.role = patch.role;
    }
