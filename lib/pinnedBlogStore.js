...
function normalizePinned(p) {
  ...
  const mediaType = ["none", "image", "video", "figure"].includes(p.mediaType) ? p.mediaType : "none";
...
export async function listPinnedPublished() {
  ...
  return store.pinned
    ...
    .sort((a, b) => b.sortOrder - a.sortOrder || String(b.dateISO).localeCompare(String(a.dateISO)));
}
